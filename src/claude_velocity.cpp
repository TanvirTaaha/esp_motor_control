...
bool IRAM_ATTR timer_group_isr_callback(void *args)
{
    ...
    left_velocity = STEPS_DIFF(current_count - prev_left_count) * VELOCITY_CALC_FREQ;
    // left_velocity = (current_count - prev_left_count) * VELOCITY_CALC_FREQ;
    prev_left_count = current_count;

    pcnt_get_counter_value(PCNT_UNIT_1, &current_count);
    right_velocity = STEPS_DIFF(current_count - prev_right_count) * VELOCITY_CALC_FREQ;
    // right_velocity = (current_count - prev_right_count) * VELOCITY_CALC_FREQ;
...
// {
//     // Print velocities
//     Serial.printf("Velocities (steps/s) - Left: %.2f, Right: %.2f ", left_velocity, right_velocity);
